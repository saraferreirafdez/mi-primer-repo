...
def main() -> int:
    ...
    p.add_argument("--enviar", action="store_true",
                   help="ENVÍA DE VERDAD. Sin esto solo simula.")
    ...
    modo = "ENVÍO REAL" if args.enviar else "SIMULACRO (no se envía nada)"
    print(f"{modo}\n{'=' * 62}")
    print(f"Día {dia} de envío · tope hoy {tope} · ya enviados {enviados_hoy(conexion)}"
          f" · quedan {restantes}\n")

    if args.enviar and not args.remitente:
    ...
    print(f"\n{'=' * 62}")
    print(f"{'Simulados' if not args.enviar else 'Enviados'}: {hechos} · saltados: {saltados}")
    if not args.enviar:
        print("\nEsto ha sido un simulacro. Revisa la lista de arriba y, si está")
        print("bien, repite el comando añadiendo --enviar")
    return 0
